...
function bu_renewals_content( $blogID ) { // SITE DASHBOARD RENEWAL CONFIRM/DENY VIEW FOR ADMINS/DETERMINED USERS; SC#3
	...
	if ( $renewed ) {
		...
		if ( $renewal < $currentDate ){ //Checks if the renewal date has passed
			print "<p>The renewal deadline of <strong>" . $end_date. "</strong> has passed. Your site will be archived for 9 months and then deleted. Please contact the Butler IT Help Desk for retrieval or any other inquires.</p>\n";
		}
		else {
			...
			print "<p>You have until <strong>" . $end_date. "</strong> to renew the website <i>'" . get_bloginfo('name') . "'</i>. Non-renewed websites will be archived for 9 months and then deleted.</p>\n";
	...
	if ( $renewed ) {
		...
		if ( $userAdmin ){
			foreach( $userAdmin as $user ){
				...
				// $message = "You have until " . $end_date. " to renew the website " . get_bloginfo('name') . ". Non-renewed websites will be archived and deleted.";
...
function bu_renewals_traffic_cop( $blogID, $type ){ // USED IN bu_renewals_add_dashboard_widget()
	...
	if ( array_intersect($allowed_roles, $user->roles) ) {
		if ( $type == "widget" ){
			...
			else if ( ($renewed == '') && isset($_REQUEST['bu-renew-site']) && ($_REQUEST['bu-renew-site'] == 'Y') ) {
