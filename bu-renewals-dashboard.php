 ...
 * @return void
 */
function bu_renewals_add_network_dashboard_widget() {

	wp_add_dashboard_widget( 'bu_renewals', 'Site Renewal Status ', 'bu_renewals_network' );
 ...
 * @return void
 */
function bu_renewals_network() {
	global $wpdb;

	# Convert into WP_Meta_Query https://developer.wordpress.org/reference/classes/wp_meta_query/?
	$siteList = $wpdb->get_results("SELECT meta_value FROM $wpdb->sitemeta WHERE meta_key LIKE 'bu_%_renewed'");
	if ( $siteList ) {
		$siteListClean = array();
 ...
 * @return void
 */
function bu_renewals_network_columns( $columns ) {
	$columns[ 'renewal' ] = __( 'Renewal' );
	return $columns;
}

add_filter( 'wpmu_blogs_columns', 'bu_renewals_network_columns' );

/**
 * bu_renewals_network_renewed()
 *
 ...
 * @return void
 */
function bu_renewals_network_renewed( $column, $blog_id ) {
	global $wpdb;
 ...
 * @return void
 */
function bu_renewals_network_user_columns( $columns ) {
 ...
 * @return void
 */
function bu_renewals_user_field( $column, $blog_id ) {
	global $wpdb;
	static $burenewal = false;
		...
		if ( $burenewal === false ) {
			...
			if ( $work ){
				foreach ( $work as $data ) {
					...
					// values[1] - renewal date
					// values[2] - user account
					$burenewal[ $values[0] ][] = $values[2];
				}
			}
		}

		if ( !empty( $burenewal[ $blog_id ] ) && is_array( $burenewal[ $blog_id ] ) ) {
			foreach( $burenewal[ $blog_id ] as $username ) {
